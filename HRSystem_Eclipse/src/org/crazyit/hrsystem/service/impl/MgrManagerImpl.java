	...
	implements MgrManager
{
	 ...
	 */
	@Override
	public void delEmp(Serializable id, String mgr) throws HrException {
		Manager m = mgrDao.findByName(mgr);
		if (m == null)
		{
			throw new HrException("您是经理吗？或你还未登录？");
		}
		empDao.delete(Employee.class, id);
	}
	

	@Override
	public void delAllEmp(List<Integer> ids, String mgr) throws HrException {
		Manager m = mgrDao.findByName(mgr);
		if (m == null)
		{
			throw new HrException("您是经理吗？或你还未登录？");
		}
		empDao.deleteAll(Employee.class, ids);
	}
		...
		throws HrException
	{
		...
		for (Employee e : emps)
		{
			result.add(new EmpBean(e.getName() ,
				e.getPass() , e.getSalary(),e.getShengao(), e.getTizhong(), e.getId()));
		}
		return result;
	}
